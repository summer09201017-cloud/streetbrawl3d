...
export const GAME_MODES = {
  duel: {
    label: "對決",
    hp: 100,
    description: "側視擂台自動面向——用八般武器打光對手的血量條就獲勝!",
    goal: "打光對手血量(各 100)",
  },
  epic: {
    label: "大戰三百回合",
...
// ---------- 比武場常數 ----------
const ARENA_HALF = 15; // 2.5D 擂台(±m,只用 X 軸)
// ★2.5D 鐵則(07-17 使用者拍板):鎖 z=0、恆自動面向對手——玩家不用控方向,
// 專心「進退距離+換武器+出招格擋」;快打旋風的深度=距離管理+時機,不是走位。
...
export class WarriorGame {
  ...
  resetFighters() {
    ...
    for (const [f, x, heading] of [[this.my, -7, Math.PI / 2], [this.foe, 7, -Math.PI / 2]]) { // 2.5D:沿 X 軸對峙開場
      f.pos.set(x, 0, 0);
      f.heading = heading;
      f.speed = 0;
      f.hp = hp;
  ...
  cycleCameraView() {
    this.cameraView = (this.cameraView + 1) % 2;
  ...
  update(delta) {
    ...
    if (!paused && this.phase === "battle") {
      ...
      // 2.5D 鎖線+恆面向(跳殺空中不轉,落地自然回正)
      for (const [f, other] of [[this.my, this.foe], [this.foe, this.my]]) {
        f.pos.z = 0;
        if (!f.leap) f.heading = other.pos.x >= f.pos.x ? Math.PI / 2 : -Math.PI / 2;
      }
  ...
  updatePlayerMovement(dt) {
    ...
    // 衝刺累計(突進技的啟動條件)
    const sprinting = this.input.isDown("sprint") && !stunned;
    ...
    if (!stunned) {
      ...
      if (dirX === facing) target = preset.maxFwd + (this.input.isDown("sprint") ? preset.boost : 0); // 朝對手=前進
      else if (dirX === -facing && dirX !== 0) target = f.speed > 0.4 ? 0 : -MAX_BACK; // 背對=後退
  ...
  movePos(f, dt) {
    ...
    // 2.5D:鎖線——z 恆 0(跳殺/推擠殘餘一併歸零)
    f.pos.z = 0;
    const nx = clamp(f.pos.x, -ARENA_HALF, ARENA_HALF);
    if (nx !== f.pos.x) f.speed *= 0.5; // 擂台邊柔性擋
    f.pos.x = nx;
  }
  ...
  updateCamera(delta) {
    ...
    if (this.phase === "menu") {
      ...
      // 2.5D 側視主鏡頭:跟兩人中點,依距離拉遠(快打旋風式)
      const sep = Math.abs(this.foe.pos.x - this.my.pos.x);
      const zoom = clamp(9.5 + sep * 0.42, 10, 17);
      desiredPos = new THREE.Vector3(mid.x, 2.9, zoom);
      desiredLook = new THREE.Vector3(mid.x, 1.5, 0);
