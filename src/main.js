...
window.__streetbrawl3d = game; // dev hook
  ...
  switch (event.type) {
    ...
    case "match-end": {
      try { if (!['localhost','127.0.0.1'].includes(location.hostname)) {   // -done:玩完一局(t=本局秒數,/stats 使用次數與平均停留吃這個)
        ...
        navigator.sendBeacon?.('https://hfpc-play-stats.summer09201017.workers.dev/api/ping?g=streetbrawl3d-done&t=' + __dt);
      } } catch (_) {}
      pushCommentary(
...
(function () {
  ...
  function _dwLeave() {
    ...
    if (s >= 3 && s <= 1800 && navigator.sendBeacon)
      navigator.sendBeacon("https://hfpc-play-stats.summer09201017.workers.dev/api/ping?g=streetbrawl3d-dwell&t=" + s);
  }
